...
class GLBLoader extends Loader {
  ...
  setDDSLoader() {
    throw new Error(
      'THREE.GLBLoader: "MSFT_texture_dds" no longer supported. Please update to "KHR_texture_basisu".'
    );
  }
  ...
  parse(data, path, onLoad, onError) {
    ...
    if (json.asset === undefined || json.asset.version[0] < 2) {
      if (onError)
        onError(
          new Error(
            "THREE.GLBLoader: Unsupported asset. glTF versions >=2.0 are supported."
          )
        );
      return;
    }
        ...
        switch (extensionName) {
          ...
          default:
            if (
              ...
              console.warn(
                'THREE.GLBLoader: Unknown extension "' + extensionName + '".'
              );
            }
        }
      }
...
class GLTFLightsExtension {
  ...
  _loadLight(lightIndex) {
    ...
    switch (lightDef.type) {
      ...
      default:
        throw new Error(
          "THREE.GLBLoader: Unexpected light type: " + lightDef.type
        );
    }
  ...
  loadTexture(textureIndex) {
    ...
    if (!loader) {
      if (
        ...
        throw new Error(
          "THREE.GLBLoader: setKTX2Loader must be called before loading KTX2 textures"
        );
      } else {
  ...
  loadTexture(textureIndex) {
    ...
    return this.detectSupport().then(function (isSupported) {
      ...
      if (
        ...
        throw new Error(
          "THREE.GLBLoader: WebP required by asset but unsupported."
        );
      }
    ...
    if (bufferView.extensions && bufferView.extensions[this.name]) {
      ...
      if (!decoder || !decoder.supported) {
        if (
          ...
          throw new Error(
            "THREE.GLBLoader: setMeshoptDecoder must be called before loading compressed files"
          );
        } else {
...
class GLTFBinaryExtension {
  constructor(data) {
    ...
    if (this.header.magic !== BINARY_EXTENSION_HEADER_MAGIC) {
      throw new Error("THREE.GLBLoader: Unsupported glTF-Binary header.");
...
class GLTFTextureTransformExtension {
  ...
  extendTexture(texture, transform) {
    if (transform.texCoord !== undefined) {
      console.warn(
        'THREE.GLBLoader: Custom UV sets in "' +
          this.name +
...
function assignExtrasToUserData(object, gltfDef) {
  if (gltfDef.extras !== undefined) {
    if (typeof gltfDef.extras === "object") {
      ...
      console.warn(
        "THREE.GLBLoader: Ignoring primitive type .extras, " + gltfDef.extras
      );
    }
  }
}
...
function updateMorphTargets(mesh, meshDef) {
  ...
  if (meshDef.extras && Array.isArray(meshDef.extras.targetNames)) {
    ...
    if (mesh.morphTargetInfluences.length === targetNames.length) {
      ...
      console.warn(
        "THREE.GLBLoader: Invalid extras.targetNames length. Ignoring names."
      );
    }
  }
}
...
function getNormalizedComponentScale(constructor) {
  ...
  switch (constructor) {
    ...
    default:
      throw new Error(
        "THREE.GLBLoader: Unsupported normalized accessor component type."
      );
  }
}
...
class GLTFParser {
  ...
  loadBuffer(bufferIndex) {
    ...
    if (bufferDef.type && bufferDef.type !== "arraybuffer") {
      throw new Error(
        "THREE.GLBLoader: " + bufferDef.type + " buffer type is not supported."
      );
    }
    ...
    return new Promise(function (resolve, reject) {
      loader.load(
        ...
        function () {
          reject(
            new Error(
              'THREE.GLBLoader: Failed to load buffer "' + bufferDef.uri + '".'
            )
          );
        }
      );
    ...
    return Promise.all(pendingBufferViews).then(function (bufferViews) {
      ...
      if (accessorDef.sparse !== undefined) {
        ...
        for (let i = 0, il = sparseIndices.length; i < il; i++) {
          ...
          if (itemSize >= 5)
            throw new Error(
              "THREE.GLBLoader: Unsupported itemSize in sparse BufferAttribute."
            );
        }
      }
    ...
    return Promise.all(pending).then(function (results) {
      ...
      for (let i = 0, il = geometries.length; i < il; i++) {
        ...
        if (
          ...
          throw new Error(
            "THREE.GLBLoader: Primitive mode unsupported: " + primitive.mode
          );
        }
          ...
          node.traverse(function (mesh) {
            ...
            for (let j = 0, jl = jointNodes.length; j < jl; j++) {
              ...
              if (jointNode) {
                ...
                console.warn(
                  'THREE.GLBLoader: Joint "%s" could not be found.',
                  skinEntry.joints[j]
                );
              }
            }
...
function computeBounds(geometry, primitiveDef, parser) {
  ...
  if (attributes.POSITION !== undefined) {
    ...
    if (min !== undefined && max !== undefined) {
      ...
      console.warn(
        "THREE.GLBLoader: Missing min/max properties for accessor POSITION."
      );
...
function toTrianglesDrawMode(geometry, drawMode) {
  ...
  if (index === null) {
    ...
    if (position !== undefined) {
      ...
      console.error(
        "THREE.GLBLoader.toTrianglesDrawMode(): Undefined position attribute. Processing not possible."
      );
      return geometry;
    }
  }
  ...
  if (newIndices.length / 3 !== numberOfTriangles) {
    console.error(
      "THREE.GLBLoader.toTrianglesDrawMode(): Unable to generate correct amount of triangles."
    );
  }
